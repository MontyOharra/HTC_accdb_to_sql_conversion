...
def getDatabaseConnections(
    ...
    if len(allSqlServerInstanceNames) == 0:
        ...
        sqlServerName = getSqlServerFullNameFromInstanceName(
            allSqlServerInstanceNames[0]
        )
    else:
        console.print(
            "[yellow]There are more than one sql servers on this machine. Please choose the server you would like to target:[/yellow]"
        )
        for instanceNumber, instance in enumerate(allSqlServerInstanceNames):
            console.print(
                f"[blue]{getSqlServerFullNameFromInstanceName(instance)} [{instanceNumber}][/blue]"
            )
        instanceNum = int(
            Prompt.ask(
                "[blue]Type the number next to the server instance you would like to select[/blue]"
            )
        )
    ...
    if htcAllPath and not htcAllPath.endswith("/"):
        htcAllPath += "/"
    if sqlDriver == None:
        useDefaultDriver = Confirm.ask(
            "[blue]Would you like to use the default ODBC driver [ODBC Driver 17 for SQL Server]?[/blue]"
        )
    ...
    if sqlDatabaseName == None:
        sqlDatabaseName = Prompt.ask(
            "[blue]Enter the what you want the SQL Server database name to be[/blue]"
        )
    ...
    databaseExists = checkIfDatabaseExists(initialSqlConn.cursor(), str(sqlDatabaseName))
    if not databaseExists:
        try:
            console.print(
                f"[yellow]The database {sqlDatabaseName} does not exist. Creating it...[/yellow]"
            )
    ...
    # Allow user input in main file
        ...
        if not autoResetDatabase:
            resetSqlDatabaseAnswer = Confirm.ask(
                "[blue]Would you like to reset the SQL Server database?[/blue]"
            )
        ...
        if resetDatabase:
            try:
                ...
                console.print(
                    f"[yellow]The database {sqlDatabaseName} exists. Resetting it...[/yellow]"
                )
            except pyodbc.Error as err:
                err = f"There was an error resetting the database"
                raise Exception(err)

    if sqlDriver is None or sqlServerName is None or sqlDatabaseName is None or htcAllPath is None:
        raise Exception("Required connection parameters are missing")
    factory = ConnFactory(str(sqlDriver), str(sqlServerName), str(sqlDatabaseName), str(htcAllPath))
...
def getMaxConversionThreads(useMaxConversionThreads: bool = False) -> int:
    ...
    if not useMaxConversionThreads:
        maxConversionThreads = Prompt.ask(
            "[blue]How many threads would you like to use for conversion? Please enter 'max' to use all available threads[/blue]"
        )
...
def getTargetTables(forceDefaultPath: bool = False) -> list[str]:
    ...
    if forceDefaultPath:
        ...
        if os.path.exists(defaultTargetTablesPath):
            useDefaultTargetTables = Confirm.ask(
                f"[blue]Found tables.txt file at '{defaultTargetTablesPath}'. Would you like to use this file[/blue]"
            )
            ...
            # User defines their own path
                ...
                targetTablesPath = Prompt.ask(
                    "[blue]Please enter the filepath for the tables you could like to convert[/blue]"
                )
        # No default path found, user must supply one
        else:
            targetTablesPath = Prompt.ask(
                "[blue]Could not find tables.txt in directory. Please enter the filepath for the tables you could like to convert[/blue]"
            )
    try:
...
def getConversionType() -> str:
    """
    Prompts the user to select the type of conversion they want to run.
    Returns either 'normalize' or 'migrate' as a string.
    """
    console = Console()
    console.print(
        "[yellow]Please select the type of conversion you would like to run:[/yellow]"
    )
    console.print("[blue]1. Normalize - Standardize and clean the data[/blue]")
    console.print("[blue]2. Migrate - Transfer data to SQL Server[/blue]")

    while True:
        choice = Prompt.ask("[blue]Enter the number of your choice (1 or 2)[/blue]")
        if choice == "1":
            return "normalize"
        elif choice == "2":
            return "migrate"
