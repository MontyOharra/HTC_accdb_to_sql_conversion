...
def readSqlCreationLog(
    htcConversionLogPath: str,
) -> dict[str, SqlCreationDetails] | None:
    """
...
def readAccessConversionLog(
    htcConversionLogPath: str,
) -> dict[str, AccessConversionDetails] | None:
...
def writeErrorLog(logPath: str, errors: dict[str, list[tuple]]) -> None:
    """
    ...
    # Process each error message to replace escaped newlines with actual newlines
    for process, errorDetails in errors.items():
        processed_errors = []
        for errorDetail in errorDetails:
            tableName, errorMessage = errorDetail
            error = f"{tableName}: {errorMessage}"
            if isinstance(error, str):
                processed_error = error.replace("\\n", "\n")
                processed_errors.append(processed_error)
            else:
                processed_errors.append(error)
        errorData[process].append(processed_errors)
...
def writeSqlCreationLog(
    logPath: str, newSqlCreationData: dict[str, SqlCreationDetails]
) -> None:
    with open(logPath, "r") as f:
        data = json.load(f)
...
def logSqlCreationProgress(
    logQueue: Queue, tableCreationData: dict[str, SqlCreationDetails], logPath: str
):
    """
    ...
    with Progress(
        ...
        while True:
            try:
                message = logQueue.get(timeout=0.1)
            except Empty:
                continue
            ...
            if isinstance(message, tuple) and len(message) == 2:
                ...
                if action == "BEGIN":
                    (tableName) = data
                    progressBar.update(
                        progressIds[tableName],
                        creationStatus="Not Started",
                        indexesStatus="Not Started",
                    )
                elif action == "UPDATE":
        ...
        while True:
            ...
            if isinstance(message, tuple) and len(message) == 2:
                ...
                if action == "BEGIN":
                    ...
                    if total > 0:
                        conversionDetails = AccessConversionDetails(
                            "Not Started", total, 0, 0
                        )
                    else:
                        conversionDetails = AccessConversionDetails(
                            "Empty Table", total, 0, 0
                        )
                    tableConversionData[tableName] = conversionDetails
                    progressBar.update(
                        progressIds[tableName],
                        conversionStatus=conversionDetails.conversionStatus,
                        completed=0,
                        errorCount=0,
                        total=total,
                    )
                    ...
                    if tableName in tableConversionData.keys():
                        ...
                        errorCountTotal = (
                            current_error_count + conversionProgress["rowErrors"]
                        )
                        rowsConvertedTotal = (
                            current_completed + conversionProgress["rowsConverted"]
                        )
                        ...
                        tableConversionData[tableName].conversionStatus = (
                            conversionStatus
                        )
                        tableConversionData[tableName].rowsConverted = (
                            rowsConvertedTotal
                        )
                    ...
                    progressBar.stop()
                    console.print(
                        f"[red]Invalid Access conversion action: {action}[/red]"
                    )
                    break
            else:
                progressBar.stop()
                console.print(
                    f"[red]Invalid Access conversion message: {message}[/red]"
                )
                break
