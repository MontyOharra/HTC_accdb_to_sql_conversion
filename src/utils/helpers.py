    ...
    if isCompiled():
        if platform.system() == "Windows":
            ...
            log_dir = os.path.join(
                os.path.expanduser("~"), ".local", "share", "HTC_Conversion", "logs"
            )
    else:
...
def chunkDictionary(
    dictionary: dict[Any, Any], maxChunkSize: int
) -> list[dict[Any, Any]]:
    """
    ...
    return [
        dict(items[i : i + maxChunkSize]) for i in range(0, len(items), maxChunkSize)
    ]
...
def generateAccessDbNameCache(tableNames: list[str]) -> dict[str, str]:
    ...
    """
    accessDbNameCache = {tableName: tableName[0:6].lower() for tableName in tableNames}
...
def generatePasswordSalt(saltLength: int) -> str:
    return secrets.token_hex(saltLength)
